...
class AdminsController extends Controller
{
    ...
    public function savesitesettings(Request $request)
    {
        ...
        if(isset($request->logo_honey)){
            $file = $request->file('logo_honey');
            $filename = $file->getClientOriginalName();
            $file->move($path, $filename);
            $logo_honey = $filename;
        }else{
            $logo_honey = $request->logo_honey_cheat;
        }

        if(isset($request->logo_meat)){
            $file = $request->file('logo_meat');
            $filename = $file->getClientOriginalName();
            $file->move($path, $filename);
            $logo_meat = $filename;
        }else{
            $logo_meat = $request->logo_meat_cheat;
        }
            ...
            if(isset($request->image)){
                ...
                if($fileSize>=1800000){
                   ...
                   Session::flash('messageError', "An error occured, You may have exceeded the maximum size for an image you uploaded");
                    ...
                    $new_timestamp = $timestamp->format('Y');
    ...
    public function page($name){
        $PagesID = DB::table('page')->where('page_name',$name)->get();

        foreach($PagesID as $value) {
            $theId = $value->id;
            $Page = Page::find($theId);

            $page_title = 'formfiletext';
            $page_name = 'Pages';
            return view('admin.page',compact('page_title','Page','page_name'));
        }
    }
    ...
    public function edit_Page(Request $request, $id){
        ...
        $updateDetails = array(
            ...
            'page_status' => $stock,
            'page_banner' =>$image_one,
            'page_featured_image_one' =>$image_two,
            'page_featured_image_two' =>$image_three,
            'page_featured_image_three' =>$image_four,
            'page_featured_image_four' =>$image_five,
        );
        DB::table('page')->where('id',$id)->update($updateDetails);
        Session::flash('message', "Changes have been saved");
        return Redirect::back();
    }
...
public function add_Product(Request $request){
    ...
    $Product->price_wholesale = $request->wholesale_price;
...
//    die();
    ...
    $updateDetails = array(
        ...
        'combo' => $request->combo,
        'wholesale' =>$wholesale,
        'price_wholesale' =>$request->wholesale_price,
        'cat' =>$request->cat,
...
public function edit_Services(Request $request, $id){
    ...
    if(isset($request->thumb)){
            ...
            $new_timestamp = $timestamp->format('Y');
            $image_main_temp = $new_timestamp.'image'.$filename;
            $thumb = str_replace(' ', '',$image_main_temp);
            $file->move($path, $thumb);

    }else{
        $thumb = $request->thumb_cheat;
    }
...
public function edit_why(Request $request, $id){
    $path = 'uploads/why';
    if(isset($request->image_one)){
   ...
   DB::table('why')->where('id',$id)->update($updateDetails);
   Session::flash('message', "Changes have been saved");
    return Redirect::back();
}
